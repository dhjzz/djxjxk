...
# КНОПКА: Вывод средств (информационная кнопка в меню)
...
async def process_withdraw_details(message: Message, state: FSMContext):
    ...
    # Списываем баланс воркера сразу, чтобы избежать спама кнопкой
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
...
async def process_user_code(message: Message, state: FSMContext):
    ...
    # При вводе кода начисляем баланс на основе прайса
...
async def process_with_refuse_reason(message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
    # Получаем сумму, чтобы вернуть её назад на баланс
